...
//     let is_menu_open = Rc::new(RefCell::new(false)); // <-- VOEG DEZE TOE
...
//     // Als de muis op de dock staat OF er staat een menu open: houd de dock zichtbaar!
...
//         let is_menu_open_show = is_menu_open.clone();
//         let is_menu_open_close = is_menu_open.clone();

//         gesture.connect_pressed(move |_, _, _, _| {
//             *is_menu_open_show.borrow_mut() = true; // Status op true wanneer menu opent
//             let popover = Popover::new();
//             popover.set_has_arrow(false);

//             let menu_flag = is_menu_open_close.clone();
//             popover.connect_closed(move |_| {
//                 *menu_flag.borrow_mut() = false; // Status op false wanneer menu sluit
//             });
...
fn build_ui(app: &Application) {
    ...
    let is_hovered_leave = is_hovered.clone();
    let is_menu_open_leave = is_menu_open.clone();
    let win_leave = window.clone();

    motion_controller.connect_leave(move |_| {
        *is_hovered_leave.borrow_mut() = false;
        check_and_update_autohide(&win_leave, false, *is_menu_open_leave.borrow());
    });
    ...
    let win_poll = window.clone();
    let is_hovered_poll = is_hovered.clone();
    let is_menu_open_poll = is_menu_open.clone();
    let container_poll = container.clone();
    let pinned_poll = pinned_apps.clone();
    let bg_poll = bg.clone();
    ...
    glib::timeout_add_local(std::time::Duration::from_millis(300), move || {
        let hovered = *is_hovered_poll.borrow();
        let menu_open = *is_menu_open_poll.borrow();
        let current_pins = pinned_poll.borrow().clone();
        let current_clients = get_running_clients();
        ...
        if *prev_state != fingerprint {
            *prev_state = fingerprint;
            render_dock_items(&container_poll, &pinned_poll, &is_menu_open_poll);
            bg_poll.queue_draw();
        }

        check_and_update_autohide(&win_poll, hovered, menu_open);
...
fn render_dock_items(
    ...
    for (index, app_info) in pins.iter().enumerate() {
        ...
        let is_menu_open_show = is_menu_open.clone();
        let is_menu_open_close = is_menu_open.clone();
        let is_menu_open_render = is_menu_open.clone();

        gesture.connect_pressed(move |_, _, _, _| {
            *is_menu_open_show.borrow_mut() = true;
            ...
            let container_inner = container_clone.clone();
            let popover_unpin = popover.clone();
            let is_menu_open_inner = is_menu_open_render.clone();
            click_unpin.connect_pressed(move |_, _, _, _| {
                pinned_apps_inner.borrow_mut().remove(index);
                save_pins(&pinned_apps_inner.borrow());
                render_dock_items(&container_inner, &pinned_apps_inner, &is_menu_open_inner);
                popover_unpin.popdown();
            });
            unpin_box.add_controller(click_unpin);
            popover_box.append(&unpin_box);
